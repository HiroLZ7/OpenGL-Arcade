...
SpaceShipThree::SpaceShipThree(int diff) {
	ship = new Ship();
	dific = diff;
	score = 0;
	kills = 0;
...
void SpaceShipThree::loadEnemy(int difVar) {
	int enemyNo = rand() % 10;
	activeEnemies.push_back(enemyProfiles.at(enemyNo));
	float enemyX = ((rand() % 200 - 100) / 100.0);
	float enemyY = ((rand() % 100) / 100.0);
	enemyProfiles.at(enemyNo).setEnemyXY(enemyX, enemyY);
	enemyProfiles.at(enemyNo).setEnemyIncrement(enemyProfiles.at(enemyNo).enemyIncrement + (difVar * .00001 * kills));
}

void SpaceShipThree::loadBoss(int difVar) {
	int bossNo = rand() % 5;
	activeEnemies.clear();
...
void SpaceShipThree::processCollisions(bool &isBoss, int difVar) {
...
int SpaceShipThree::runSpaceDefender(void) {
	...
	GLFWwindow* window = glfwCreateWindow(600, 600, "Space Defender 3", NULL, NULL);
	if (!window) {
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
